...
```{r, message=FALSE, warning=FALSE, fig.width=30, fig.height=30}
plot_list <- lapply(celltypes, function(ds){
...
pheatmap::pheatmap(bwr@scores, cellheight = 15, cellwidth = 15)
```
...
pheatmap::pheatmap(bwr@scores$NES, cellheight = 15, cellwidth = 15)
```

And now the FDR, remembring that `<0.25` is usually the cut off.
```{r, message=FALSE, warning=FALSE, fig.width=15, fig.height=10}
pheatmap::pheatmap(bwr@scores$padj, cellheight = 15, cellwidth = 15, color = viridis::viridis(50, direction = -1))
```
